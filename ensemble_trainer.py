...
class EnsembleTagger(SequenceTagger):
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            for batch in copy.deepcopy(data_loader):
                batch_no += 1
...
class ModelStackTrainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        log_line(log)
        log.info(f'Model: "{self.model_stack}"')
        log_line(log)
        ...
        for i in range(self.model_size):
            optimizer: torch.optim.Optimizer = self.optimizer(
                self.model_stack[i].parameters(), lr=learning_rate, **kwargs
            )
            opt_stack.append(optimizer)
        ...
        try:
            ...
            for self.epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                for group in opt_stack[0].param_groups:
                ...
                #     self.model_stack.load_state_dict(
                ...
                self.model_stack[0].train()
                self.model_stack[1].train()
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    for i in range(self.model_size):
                        self.model_stack[i].zero_grad()
                        opt_stack[i].zero_grad()
                    ...
                    for batch_step in batch_steps:
                        ...
                        losses = []
                        for i in range(self.model_size):
                            losses.append(self.model_stack[i].forward_loss(copy.deepcopy(batch_step)))
                    ...
                    for i in range(self.model_size):
                        torch.nn.utils.clip_grad_norm_(self.model_stack[i].parameters(), 5.0)
                        opt_stack[i].step()
                    ...
                    if seen_batches % modulo == 0:
                        ...
                        if not param_selection_mode:
                            weight_extractor.extract_weights(
                                self.model_stack[0].state_dict(), iteration
                            )
                ...
                if log_train:
                    for i in range(self.model_size):
                        train_eval_result, train_loss = self.model_stack[i].evaluate(
                            DataLoader(
                                self.corpus.train,
                ...
                if log_train_part:
                    for i in range(self.model_size):
                        train_part_eval_result, train_part_loss = self.model_stack[i].evaluate(
                            DataLoader(
                                train_part,
                                batch_size=mini_batch_chunk_size,
                                num_workers=num_workers,
                ...
                if log_dev:
                    for i in range(self.model_size):
                        dev_eval_result, dev_loss = self.model_stack[i].evaluate(
                            DataLoader(
                                self.corpus.dev,
                                batch_size=mini_batch_chunk_size,
                                num_workers=num_workers,
                ...
                if log_test:
                    for i in range(self.model_size):
                        test_eval_result, test_loss = self.model_stack[i].evaluate(
                            DataLoader(
                                self.corpus.test,
                                batch_size=mini_batch_chunk_size,
                                num_workers=num_workers,
                ...
                for i in range(self.model_size):
                    sche_stack[i].step(current_score)
                ...
                for i in range(self.model_size):
                    ...
                    try:
                        bad_epochs = sche_stack[i].num_bad_epochs
                    except:
                        bad_epochs = 0
                    for group in opt_stack[i].param_groups:
                ...
                if (
                    ...
                    for i in range(self.model_size):
                        self.model_stack[i].save(base_path / ("best-model-%d.pt"%i))

            # if we do not use dev data for model selection, save final model
            if save_final_model and not param_selection_mode:
                for i in range(self.model_size):
                    self.model_stack[i].save(base_path / ("final-model-%d.pt"%i))
            ...
            if not param_selection_mode:
                ...
                for i in range(self.model_size):
                    self.model_stack.save(base_path / ("final-model-%d.pt"%i))
                log.info("Done.")
    ...
    def load_checkpoint(cls, checkpoint: Union[Path, str], corpus: Corpus):
        model: ModelStackTrainer = torch.load(checkpoint, map_location=flair.device)
        model.corpus = corpus
        return model
    ...
    def final_test(
        ...
        if (base_path / "best-model.pt").exists():
            self.model_stack = []
            for i in range(self.model_size):
                self.model_stack.append(EnsembleTagger.load(base_path / ("best-model-%d.pt"%(i))))

        final_score = []
        for i in range(self.model_size):
            test_results, test_loss = self.model_stack[i].evaluate(
                DataLoader(
                    self.corpus.test,
            ...
            final_score.append(test_results.main_score)
